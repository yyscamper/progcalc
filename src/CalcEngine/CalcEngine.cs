	...
	public partial class CalcEngine
	{
        ...
        Dictionary<object, Token> GetSymbolTable()
        {
            if (_tkTbl == null)
            {
                ...
                AddToken("<<", TKID.LSHIFT, TKTYPE.SHIFT);
                AddToken(">>", TKID.RSHIFT, TKTYPE.SHIFT);
		...
		{
			GetToken();
			return ParseLogicBitsOr();
		}

        Expression ParseLogicBitsOr()
        {
            var x = ParseLogicBitsAnd();
            while (_token.Type == TKTYPE.LOGIC_BITS_OR)
            {
                var t = _token;
                GetToken();
                var exprArg = ParseLogicBitsAnd();
                x = new BinaryExpression(t, x, exprArg);
            }
            return x;
        }

        Expression ParseLogicBitsAnd()
        {
            var x = ParseCompare();
            while (_token.Type == TKTYPE.LOGIC_BITS_AND)
            {
                var t = _token;
                GetToken();
        ...
        {
            var x = ParseAddSub();
            while (_token.Type == TKTYPE.SHIFT)
            {
                var t = _token;
                GetToken();
